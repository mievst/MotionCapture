...
class RigHandler:
    ...
    def __init__(self, rig_name : str,):
        ...
        self.base_bone_loc, _, self.base_bone_sca = (self.rig.matrix_world @ self.rig.pose.bones["torso"].matrix).decompose()

    base_rig_bones_openvino = {
        #"neck": "neck",
        #"right_shoulder": "shoulder.R",
        #"right_elbow": "forearm_tweak.R",
        "right_wrist": "hand_ik.R",
        #"left_shoulder": "shoulder.L",
        #"left_elbow": "forearm_tweak.L",
        ...
        #"nose": None,
        "left_hip": None,
        #"left_knee": "shin_tweak.L",
        "left_ankle": "foot_ik.L",
        "right_hip": None,
        #"right_knee": "shin_tweak.R",
    ...
    def set_torso_data(self,
                    pose : dict,
                    l_hip_bone_name : str,
                    r_hip_bone_name : str,
                    right_shoulder_bone_name : str,
                    left_shoulder_bone_name : str):
        l_hip = self.get_loc(pose, l_hip_bone_name)
        r_hip = self.get_loc(pose, r_hip_bone_name)
        r_shoulder = self.get_loc(pose, right_shoulder_bone_name)
        l_shoulder = self.get_loc(pose, left_shoulder_bone_name)
        ...
        bpy.context.view_layer.update()
        self.rig.location = center
        self.rig.location.z = 0
        bpy.context.view_layer.update()
    ...
    def set_frame(self, pose : dict):
        ...
        self.set_torso_data(pose, "left_hip", "right_hip", "right_shoulder", "left_shoulder")
        #self.set_shoulder_data(pose, "l_shoulder", "r_shoulder")
        #for key, val in self.base_rig_bones_openvino.items():
        for key, val in self.base_rig_bone_mediapipe.items():
            if val == None:
                continue
    ...
    def find_relation(self, poses : list[dict]):
        max_box = max(poses, key=lambda x: x["box"]['y_max'] - x["box"]['y_min'])["box"]
        high = (max_box["y_max"] - max_box["y_min"]) * 0.6
